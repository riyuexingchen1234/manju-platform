...
public class SceneService {
    ...
    public SceneGenerateResponse generateScene(SceneGenerateRequest req){
        // 1. 检查用户是否存在
        ...
        if (!freeUsed) {
            ...
            if (user.getPoints() < Constants.POINTS_SCENE_GENERATE) {
                ...
                failLog.setToolName(Constants.TOOL_SCENE_GENERATE);
                failLog.setIsFree(0);
                failLog.setPointsDeduct(0);
                failLog.setCallStatus(0);
                failLog.setFailReason("积分不足，需要" + Constants.POINTS_SCENE_GENERATE + "分");
                logDao.insert(failLog);
                throw new RuntimeException("积分不足");
            }
            // 乐观锁扣积分（返回新积分，但不需要使用）
            int newPoints = deductPointsWithRetry(req.getUserId(), Constants.POINTS_SCENE_GENERATE, 3);

            // 记录付费成功日志
            UsageLog successLog = new UsageLog();
            successLog.setUserId(req.getUserId());
            successLog.setToolName(Constants.TOOL_SCENE_GENERATE);
            successLog.setIsFree(0);
            successLog.setPointsDeduct(Constants.POINTS_SCENE_GENERATE);
